...
- **Constant-pH MD (the rigorous version): BLOCKED on toolchain — engine decision below.** Verified directly (2026-06-25, not inferred): the project's `gmx_mpi` is **GROMACS 2025.0 mainline** and `strings libgromacs_mpi.so.10` has **zero** `constant-ph`/`cphmd`/`titratable`/`lambda-dynamics` symbols (vs `tcoupl`/`pcoupl` which are present), and the binary reports *"alchemical lambda dynamics is not supported by this build."* Native constant-pH is not in mainline GROMACS; it lives in a separate fork. CpHMD would give the per-linker local pKa (the dynamic version of the §25.6 static NULL), still a proxy — not the bond-breaking reaction (= QM/MM).

  **Three engines were evaluated; the FF-reparametrization cost is decisive (the system is AMBER-native: protein amber99sb-ildn, ligand GAFF2/AM1-BCC; acpype already emitted the native AMBER `LIG_AC.prmtop`/`.frcmod` under `md/lig_params/*/LIG.acpype/`).**

  - **(A) GROMACS-constantph fork + phbuilder** — *install path:* build the constant-pH GROMACS fork (a patched GROMACS ~2021, kept SEPARATE from the 2025 install; repo + tutorials `gitlab.com/gromacs-constantph`); `pip install phbuilder` (Jansen et al., *JCIM* 2024, 10.1021/acs.jcim.3c01313); set the titratable-group definitions in `lambdagrouptypes.dat`; then `phbuilder gentopol` (assign λ states) → `phbuilder neutralize` (add the buffer ions) → `phbuilder genparams` (write the constant-pH `.mdp`) → `gmx grompp`/`mdrun` with the fork. **Blocker:** this implementation *only supports a modified CHARMM36m force field* — using it forces re-parametrizing the protein to CHARMM36m AND the custom thiosuccinimide ligand to CGenFF, discarding the validated AMBER/GAFF2/AM1-BCC setup. So its apparent "stay in GROMACS" advantage is illusory.
  - **(B) AMBER constant-pH (AmberTools)** — *path:* `tleap` (protein ff14SB/ff19SB ≈ amber99sb-ildn lineage; load the existing GAFF2 ligand `mol2`+`frcmod`; recreate the Cys214 Sγ–ligand-C covalent bond with the `bond` command) → `cpinutil.py` to define the titratable residues (the proximal Asp/Glu/His/Lys/Cys around Cys214 — the §20.13 / §25.6 observable) → constant-pH `mdin` (`icnstph`, `solvph`) → `sander` (CPU, free in AmberTools) or `pmemd.cuda` (GPU, needs an Amber license) → analyse with `cphstats`. **Zero reparametrization** (the AMBER ligand topology already exists). Most mature/validated CpHMD for residue pKa.
  - **(C) CHARMM CpHMD (native λ-dynamics, Brooks lab)** — most historically-validated, but requires CHARMM (license) + full CHARMM36m/CGenFF reparametrization. Only sensible if the project were already CHARMM-based; it is not.

  **VERDICT: AMBER constant-pH (AmberTools, CPU `sander` for the 2–3 candidate + vedotin pilot).** Reasoning: both the GROMACS fork and CHARMM mandate a full CHARMM36m+CGenFF reparametrization of the protein AND the custom covalent ligand — re-opening parametrization the project already made and validated (stereo §19.24, valence §20.11) and incurring a revalidation burden; AMBER alone preserves the native FF (the `LIG_AC.prmtop` is on disk), and AMBER CpHMD is the reference implementation for the exact observable we want. The fork's only real edge (free + GPU + GROMACS file ecosystem) is negated by the CHARMM36m-only constraint. Accepted tradeoff: free AMBER CpHMD is CPU-only (`pmemd.cuda` GPU needs a license) — acceptable for a small pilot. Until either is set up, the static PROPKA pilot (§25.6, NULL) is the runnable surrogate.

  **Scaffold written: `experiments/wave2/cphmd/`** — `run_cphmd.sh` (rename ASP/GLU/HIS→AS4/GL4/HIP → `tleap` covalent build `bond Cys.SG–LIG.attachC` → `cpinutil.py` cpin → min/heat/equil → explicit-solvent `prod_cph` `icnstph=2, ntcnstph=100, solvph=<pH>` over a pH ladder → `cphstats`) + `select_titratable.py` (validated: returns the proximal titratable set GLU213/ASP122/GLU219/LYS136/TYR186 around Cys214 — matches §25.6) + README. Fail-closes when AmberTools is absent (no fake runs); the only manual per-candidate step is resolving `CYS_RESNUM` (tleap residue index of the conjugation Cys) and `LIG_ATTACH` (ligand attach-carbon atom name in the mol2). Ready to run once `conda install -c conda-forge ambertools` is done.
...
### §25.9 — AMBER26 installed + pmemd.cuda GPU built + GB CpHMD GPU pilot EXECUTED (2026-06-25)

Executing the §25.4 decision (AMBER engine): the full GPU CpHMD toolchain was installed and the cand_4 conjugate built, valence-validated, and simulated on the RTX 4080. Reproducible recipe + data below.

**1. AmberTools26 (setup/analysis), conda binary → `/home/galeito/at26`.** conda's libmamba solver is broken here (`libarchive.so.19` missing) and the classic solver is too slow for ambertools, so use a standalone micromamba binary: fetch it from GitHub releases (`mamba-org/micromamba-releases/.../micromamba-linux-64`; the `micro.mamba.pm` curl-pipe gets killed by the task harness) and **invoke it under the exact name `micromamba`** (`/home/galeito/mm/micromamba`; a differently-named binary fails env activation). Then `micromamba create -p /home/galeito/at26 -c dacase -c conda-forge ambertools-dac=26`. Gives tleap / cpinutil.py / cphstats / sander(CPU) / parmed + GPU QM/analysis (quick.cuda, cpptraj.cuda, sander.quick.cuda) but **NO pmemd.cuda** (the licensed classical-MD GPU engine).

**2. Covalent conjugate (AMBER-native; full recipe in `experiments/wave2/cphmd/BUILD_VALIDATED.md`).** tleap with ff14SB + `leaprc.constph` + gaff2; `pdb4amber --nohyd`; Cys214→**CYX**; proximal carboxylates GLU213/ASP122/GLU219 → GL4/AS4; ligand H76 removed + charge folded into C48; **junction.frcmod** (cross-FF S–c5 bond/angle/dihedral terms, C–S 1.81 Å); `bond CYX.SG – LIG.C48`. Validated: SG 2-bonds / C48 4-bonds (no §20.10/§19.28 valence bugs). GB system `complex_gb.prmtop` = **15,828 atoms** + `cpin_gb` (3 titratable); explicit system `complex_solv` = 280,111 atoms (neutral) also built.

**3. pmemd.cuda 26 (GPU engine) built from the user's `pmemd26.tar.bz2`** (free non-commercial, self-contained, md5 `ceeabc…`; the user's Amber22/pmemd22 was the WRONG version for AmberTools26). Recipe:
- Compilers via micromamba: `micromamba create -p /home/galeito/amber_build -c conda-forge gcc=13 gxx=13 gfortran=13 make flex bison`. (conda-forge default gcc 15 is too new — CUDA 12.8 supports gcc ≤14 → pin **gcc 13**.)
- `tar xjf pmemd26.tar.bz2` → `pmemd26_src/` (self-contained: bundles its own `AmberTools/` build subdir + the CUDA `cub/` kernels).
- Edit `pmemd26_src/build/run_cmake`: `-DCUDA=FALSE`→`-DCUDA=TRUE` (keep `-DPMEMD_ONLY=TRUE -DMPI=FALSE`); run under the compiler env with CUDA 12.8 on PATH (`CUDA_HOME=/usr/local/cuda-12.8`) via `micromamba run -p /home/galeito/amber_build`. Use **system cmake 3.22.1** (the manual forbids a conda cmake). cmake report: "Building the GTI version of pmemd.cuda", "CUDA 12.8 ↔ GNU compatible", arch **sm_89**.
- `make install -j4` (~45 min; CUDA kernels dominate). Builds SPFP + DPFP → `/home/galeito/Amber26/pmemd26/bin/pmemd.cuda` (symlink→_SPFP) + amber.sh.

**4. GPU test (DATA).** `pmemd.cuda` smoke on the 15.8k GB conjugate: **NVIDIA RTX 4080, 100 MD steps clean, 53.6 ns/day, exit 0.** Confirms the binary works on GPU and the two session-teardown interruptions left no corrupt object (incremental `make` resumed from the .o tree each time).

**5. GB CpHMD GPU pilot (EXECUTED).** pmemd.cuda GB constant-pH titration of cand_4: pH **{3,4,5,6,7} × 2 ns**, mdin `icnstph=1, igb=2, cut=9999.0, saltcon=0.1, ntcnstph=5, ntt=3, temp0=300, ntc=2/ntf=2` (cut=9999 because pmemd.cuda GB rejects a finite cutoff — infinite cutoff is valid per the manual and consistent with the cpin reference energies). GPU ~93 % util. Outputs `cpout_gpu_pH{N}`; analysis `cphstats -i cpin_gb cpout_gpu_pHN` → fraction-protonated vs pH → per-residue pKa for GLU213/ASP122/GLU219 = the **dynamic, per-linker** version of the §25.6 static-PROPKA NULL. Validated against Amber26 manual Ch.28: our mdin matches the cpin reference-energy conditions (`cut/igb/saltcon/nrespa/temp0/ntc/ntf`); ff14SB is charge-compatible with the ff99SB-derived references.
